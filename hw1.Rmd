...
mixture_of_Gaussians <- function(j, omega, x){
  return(1)
}

take_sample <- function(centroid, N, pi, omega, sigma2, ys){
  X <- matrix()
  for(y in ys){
    #placeholder
  }
...
y_train <- sample(c(1,0), train_size, TRUE)
y_test <- sample(c(1,0), test_size, TRUE)
```
...
X <- runif(n,0,1)
X2 <- X^2
y_noiseless <- sapply(X, function(x) {1+x^2})
